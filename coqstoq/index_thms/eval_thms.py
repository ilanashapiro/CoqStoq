...
def check_valid_coqpyt(project: Project, path: Path, timeout: Optional[int]) -> bool:
    ...
    with CoqFile(
        ...
        coq_file.run()
        return coq_file.is_valid


def find_eval_theorems(
    project: Project, path: Path, timeout: Optional[int]
) -> list[EvalTheorem]:
    if not check_valid_coqpyt(project, path, timeout):
        raise CoqComplieError(f"File {path} is not a valid Coq file.")
    str_file_path = str(path.resolve())
    str_workspace_path = str(project.workspace.resolve())
