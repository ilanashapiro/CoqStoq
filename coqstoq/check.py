...
def get_theorem_statement(thm: EvalTheorem, coqstoq_loc: Path) -> str:
    orig_file_loc = coqstoq_loc / thm.project.workspace / thm.path
    assert orig_file_loc.exists()
    assert (
        get_file_hash(orig_file_loc) == thm.hash
    ...
    proof_lines[-1] = proof_lines[-1][: thm.theorem_end_pos.column]
    proof_lines[0] = proof_lines[0][thm.theorem_start_pos.column :]
    return "\n".join(proof_lines)
